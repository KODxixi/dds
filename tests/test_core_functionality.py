...
async def test_1_never_empty():
    ...
    assert section.is_complete(), "所有必填字段都应该已填充"
    assert len(section.data) > 0, "数据不应该为空"
    assert section.confidence["score"] > 0, "置信度应该大于 0"

    print("\n✓ 测试 1 通过：数据永远不为空，缺失时自动降级")
...
async def test_2_all_sections_complete():
    ...
    for section_id, section in all_sections.items():
        ...
        print(f"  {section_id}: {len(section.data)} 字段, "
              f"真实数据 {real_count}, "
...
async def test_3_contract_enforcer():
    ...
    assert result.valid, "完整数据应该通过校验"
...
async def test_4_mixed_data():
    ...
    print(f"  ✓ 真实数据保留: project_id = {section.data['project_id']}")
    print(f"  ✓ 真实数据保留: decision_question = {section.data['decision_question'][:40]}...")

    # 缺失字段有降级值
    assert "evidence_boundary" in section.data
    assert section.data_origin["evidence_boundary"].source != "real"
    print(f"  ✓ 自动填充缺失字段: evidence_boundary = {section.data_origin['evidence_boundary'].label}")
...
async def test_5_data_quality_report():
    ...
    if quality_report['summary']['fallback_by_strategy']:
        print(f"  降级策略分布:")
        for strategy, count in quality_report['summary']['fallback_by_strategy'].items():
