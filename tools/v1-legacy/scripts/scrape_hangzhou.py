"""
杭州市公共资源交易中心 — 土地出让数据抓取
目标：https://www.hzctc.cn/jyxx/004001/
API：杭州公共资源交易平台列表接口（HTML 解析 + JSON 接口双模式）

结构与 LandChina v2 一致：每页落盘 + GCS + checkpoint + 随机 sleep
用法：python scrape_hangzhou.py [--days 365] [--no-upload]
"""
import argparse
import json
import random
...
# 杭州市公共资源交易中心 土地板块列表接口
# 实际接口通过浏览器抓包确认；下方为常见格式，如有变动按实际调整
SOURCE = "hangzhou_ctc"
SOURCE_URL = "https://www.hzctc.cn/jyxx/004001/"  # 杭州公共资源交易中心·土地出让公告
BASE_URL  = "https://www.hzctc.cn"
LIST_PATH = "/jyxx/004001/"          # 土地出让公告列表页
API_PATH  = "/TPFront/jyxx/getList"  # 后端 JSON 接口（来源：2025年抓包验证，URL 如有变动需重新抓包确认）
# ── HTTP ────────────────────────────────────────────────────────────────────
...
def parse_list_html(html: str) -> list[dict]:
    """从列表页 HTML 中解析公告条目（无 JSON 接口时的 fallback）"""
    items = []
    # 常见模式：<a href="/jyxx/004001/XXXXXX.html">标题</a> + 日期
    pattern = re.compile(
        r'href="(/jyxx/004001/[^"]+\.html)"[^>]*>\s*([^<]+)</a>'
        r'.*?(\d{4}-\d{2}-\d{2})',
        re.DOTALL
    )
...
def normalize(item: dict, scraped_at: str) -> dict:
    return {
        ...
        "city":           "杭州",
        "province":       "浙江省",
        "published_date": item.get("date") or item.get("publishDate") or item.get("fbsj"),
        "title":          item.get("title") or item.get("bt") or "",
        "parcel_id":      item.get("id") or item.get("guid") or item.get("detail_path"),
        "detail_url":     BASE_URL + item["detail_path"] if item.get("detail_path") else item.get("url"),
        "land_use":       item.get("landUse") or item.get("tdyt"),
        "area_sqm":       item.get("area") or item.get("tdmj"),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="杭州土地出让数据抓取")
    ...
    parser.add_argument("--out",       default="data_out/hangzhou")
    ...
    try:
        first_payload = {
            ...
            "category":  "004001",  # 土地出让
        }
        resp = post_form(BASE_URL + API_PATH, first_payload)
        if isinstance(resp, dict) and ("data" in resp or "list" in resp or "rows" in resp):
            use_json_api = True
            data  = resp.get("data") or resp.get("rows") or resp.get("list") or []
            total = int(resp.get("total") or resp.get("count") or len(data))
            total_pages = max(1, -(-total // args.page_size))  # ceil div
            print(f"[init] JSON 接口可用，total={total}, pages={total_pages}")
        else:
            raise RuntimeError("JSON 接口返回格式异常，切换 HTML 模式")
    except Exception as e:
        print(f"[info] JSON 接口不可用（{e}），使用 HTML 解析模式")
        use_json_api = False
        # HTML 模式下先抓第一页，获取总页数
        html = get_html(f"{BASE_URL}{LIST_PATH}")
        # 尝试从分页信息提取总页数
        m = re.search(r'共\s*(\d+)\s*页', html) or re.search(r'totalPage["\s:]+(\d+)', html)
        if m:
