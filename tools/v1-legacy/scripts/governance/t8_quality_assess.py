...
def assess_quality(rows: list[dict]) -> dict[str, Any]:
    ...
    if overall < 60:
        try:
            ...
            # 找出最差维度
            worst_dim = min(dimensions.items(), key=lambda x: x[1]["score"])
            worst_name = worst_dim[0]
            worst_score = worst_dim[1]["score"]
            notifier.send(
                module="t8_quality_assess",
                level="orange",
                title="T8 质量评估分数偏低",
                summary=f"综合分：{overall}，等级：{grade}，"
                        f"最差维度：{worst_name}({worst_score}分)",
                detail={
                    "overall_score": overall,
                    "grade": grade,
                    "total_rows": len(rows),
                    "dimensions": {d: info["score"] for d, info in dimensions.items()},
                },
            )
        except Exception:
            pass  # 通知失败不影响质量评估主流程
