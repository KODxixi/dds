...
def p0_gate_with_ge(rows: list[dict]) -> tuple[bool, list[dict[str, Any]]]:
    """
    P0 入库前门禁（含 GE 声明式验证）：质量门禁 5 项 + GE P0 Suite 检查。

    参数:
        rows: 行列表

    返回:
        (passed, checks) — passed=True 则可入库
    """
    # 原有 P0 5 项检查
    checks = [
        check_schema_columns(rows),
    ...
    # GE 声明式 P0 验证
    try:
        from ge_quality_gates import ge_p0_gate
        ge_result = ge_p0_gate(rows)
        # 将 GE 结果作为额外检查项追加
        checks.append({
            "check": "ge_p0_suite",
            "passed": ge_result["passed"],
            "detail": ge_result["summary"],
            "severity": "block" if not ge_result["passed"] else "pass",
            "ge_details": ge_result,
        })
    except ImportError:
        checks.append({
            "check": "ge_p0_suite",
            "passed": True,
            "detail": "GE 模块未加载，跳过（pip install ge_quality_gates 或检查路径）",
            "severity": "pass",
        })
...
def run_gate(
    ...
    **kwargs: Any,
    ...
    if level == "P0":
        if use_ge:
            passed, checks = p0_gate_with_ge(rows)
        else:
    ...
    if not passed:
        try:
            ...
            pass_rate = f"{result['passed_checks']}/{result['total_checks']}"
            # 按门禁级别确定通知参数
            if level == "P0":
                notify_level = "red"
                notify_title = "P0 入库门禁阻断"
            elif level == "P1":
                notify_level = "orange"
                notify_title = "P1 日度门禁告警"
            else:  # P2
                notify_level = "yellow"
                notify_title = "P2 周度门禁质量下降"
            notifier.send(
                module="quality_gates",
                level=notify_level,
                title=notify_title,
                summary=f"失败检查项：{', '.join(failed_names)}，通过率：{pass_rate}",
                detail=result,
            )
        except Exception:
            pass  # 通知失败不影响门禁主流程
...
def main() -> None:
    ...
    parser.add_argument("--ge", action="store_true",
                        help="启用 GE 声明式验证（P0 级别追加 dds_governance_p0 Suite）")
    ...
    result = run_gate(rows, level=args.level, use_ge=args.ge, **kwargs)
    ...
    ge_tag = " [GE]" if args.ge else ""
    print(f"\n[gate] {result['level']}{ge_tag} 门禁结果：{status}（动作：{action}）")
