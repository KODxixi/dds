...
def _asset_path_candidates(locator: str) -> list[Path]:
    if not locator or re.match(r"^(?:https?|tos|s3)://", locator, re.I):
        return []
    source = Path(locator)
    ...
    if not source.is_absolute():
        candidates.extend(
            [
                root / source,
                Path(r"D:\ArchLib") / source,
                Path(r"D:\ArchLib\_project") / source,
            ]
        )
    result: list[Path] = []
    seen: set[str] = set()
...
def _resolved_asset_registry(document: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for asset_id in ordered_ids:
        ...
        if not data_uri and (
            ...
            locator = str(entry.get("data_or_object_ref") or "")
            for candidate in _asset_path_candidates(locator):
...
def resolve_cinematic_document(report_json: Mapping[str, Any]) -> dict[str, Any]:
    ...
    document["asset_registry"] = _resolved_asset_registry(document)
...
def render_cinematic_deck_html(report_json: dict) -> str:
    """Render a dependency-free HTML shell around a virtual PageManifest."""
    if not isinstance(report_json, Mapping):
        raise TypeError("report_json must be a mapping")
    document = resolve_cinematic_document(report_json)
    ...
    metadata = {
        ...
        "asset_registry": _resolved_asset_registry(document),
