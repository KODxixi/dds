...
from query_local import (
    ...
    VAULT,
    list_pool_cities,
    preferred_duckdb_source,
)
from geo_clean import clean_coords

SALE_STATUSES = ("在售", "尾盘")
REAL_HISTORY_MARKER = "来源:购买库按真实开盘年"


def mark_history_provenance(frame: pd.DataFrame, year: str) -> pd.DataFrame:
    """按行内溯源标记区分真实历史回填与旧合成数据。"""
    frame = frame.copy()
    ...
    notes = frame["source_note"].fillna("").astype(str)
    frame["is_synthetic"] = ~notes.str.startswith(REAL_HISTORY_MARKER)
    return frame
            ...
            for year in years:
                ...
                for candidate in candidates:
                    if candidate.exists():
                        ...
                        current_state = None
                    if file_mtimes.get(str(candidate)) != current_state:
    ...
    def set(self, cities: tuple, years: tuple, df: pd.DataFrame):
        ...
        for city in cities:
            for year in years:
                path = csv_path_for_city(city, year)
                if path:
                    for candidate in (path, path.with_suffix(".parquet")):
                        if candidate.exists():
                            stat = candidate.stat()
                            file_mtimes[str(candidate)] = (stat.st_mtime_ns, stat.st_size)
                        else:
                            file_mtimes[str(candidate)] = None
...
def csv_path_for_city(city: str, year: str = None) -> Path | None:
    ...
    if target_year == CURRENT_YEAR:
        p_dynamic = VAULT / "2026新楼盘" / f"新楼盘-{city}.csv"
        if p_dynamic.exists():
            return p_dynamic
...
def load_projects(cities: list[str] | None = None, year: str = None,
    ...
    """
    selected = cities or sorted(list_pool_cities())
    ...
    for city in selected:
        for ty in target_years:
            ...
            read_source = preferred_duckdb_source(csv_path)
            ...
            df = duckdb.query(f"""
                ...
                FROM {read_source}
            """).df()
            df = mark_history_provenance(df, ty)
            frames.append(df)
...
def _local_match(city: str, address: str) -> dict | None:
    """本地降级匹配：街道门牌 → 行政区 → 全市，逐级放宽，返回最精确命中。
    用于高德在线编码不可用，或在线结果精度过粗时补一层街道级定位。"""
    import re
    projects = load_projects([city], year="2026")
    if projects.empty:
        return None
    addr_col = projects["address"].astype(str)
    name_col = projects["project_name"].astype(str)
    # 真实行政区（数据集已知区名反向匹配，"海棠区"∈"三亚海棠区南田路16号"）
    known = [d for d in projects["district"].dropna().astype(str).unique() if d]
    hit = next((d for d in known if d in address), None)
    # 剥离市名+区名，得到街道门牌核心串（"三亚海棠区南田路16号"→"南田路16号"）
    core = address.replace(city, "")
    if hit:
        core = core.replace(hit, "")

    matched = projects.iloc[0:0]  # 空集
    precision = "city"
    # L1 整串/名称包含（最精确）
    m = projects[addr_col.str.contains(re.escape(address), na=False) |
                 name_col.str.contains(re.escape(address), na=False)]
    if not m.empty:
        matched, precision = m, "address"
    # L2 街道门牌匹配（取"南田路"，可命中地址含"海棠湾"的项目，街道级精度）
    if matched.empty:
        st = re.search(r"[一-龥]{1,6}(?:路|街|大道|道|巷|村|农场)", core)
        if st:
            m = projects[addr_col.str.contains(re.escape(st.group(0)), na=False)]
            if not m.empty:
                matched, precision = m, f"street:{st.group(0)}"
    # L3 行政区
    if matched.empty and hit:
        matched, precision = projects[projects["district"].astype(str) == hit], f"district:{hit}"
    # L4 退回全市
    if matched.empty:
        matched = projects
    ...
    if matched.empty:
        return None
    dvals = matched["district"].dropna().astype(str)
    return {
        "lng": float(matched["lng"].median()),  # 中位数抗离群
        "lat": float(matched["lat"].median()),
        "precision": precision,
        "district": hit or (dvals.iloc[0] if not dvals.empty else None),
        "n": int(len(matched)),
    }
...
def resolve_location(city: str, address: str | None, lng: float | None, lat: float | None, key: str) -> dict:
    ...
    import re
    # 1. 高德在线地理编码：仅"门牌/道路/兴趣点"等精确档直接采用；省/市/区县等粗档需本地补正
    COARSE = {"国家", "省", "市", "区县", "开发区", "乡镇", "村庄", "热点商圈"}
    geo = None
    try:
    ...
    def _dist_from(s):  # 从格式化地址抽行政区名，作为 district（先去"X市"前缀避免贪婪误匹配）
        mm = re.search(r"[一-龥]{2,3}区", (s or "").split("市")[-1])
    ...
    def _geo_result():
        return {
            ...
            "level": geo.get("level"),
            "district": _dist_from(geo.get("formatted_addr")),
            "adcode": geo.get("adcode"),
        }

    # 在线精确（level 非空且不在粗档集合）→ 直接采用
    if geo and geo.get("level") and geo.get("level") not in COARSE:
        return _geo_result()

    # 2. 本地降级匹配（在线失败，或在线精度过粗时补一层街道级）
    local = None
    try:
        local = _local_match(city, address)
    except Exception as inner_e:
        print(f"[warn] 离线匹配代理地址失败: {inner_e}")

    def _local_result():
        return {
            "lng": local["lng"], "lat": local["lat"],
            "source": "offline_proxy_match",
            "formatted_addr": f"离线匹配[{local['precision']}] {city}: 基于 {local['n']} 个楼盘中位坐标",
            "level": local["precision"],
            "district": local["district"],
            "adcode": None,
        }

    # 2a. 本地有整串/街道级命中 → 优先本地（比粗精度在线更准）
    if local and local["precision"].split(":")[0] in ("address", "street"):
        return _local_result()
    # 2b. 在线虽粗但有结果 → 用在线（聊胜于无）
    if geo:
        return _geo_result()
    # 2c. 在线全无 → 用本地任意层（区/全市兜底）
    if local:
        return _local_result()

    raise RuntimeError(f"高德在线编码失败且本地无法匹配到任何代理坐标点：{address}")
...
def analyze_price_band(projects: pd.DataFrame, expected_price: float | None, price_band: float) -> dict | None:
    ...
    return {
        ...
        "sample_scope": "本地五城样本：三亚、杭州、上海、青岛、济南",
...
def build_report(args) -> dict:
    ...
    projects = load_projects([args.city], year=year, multi_year=False)
    if projects.empty:
...
def main() -> int:
    ...
    try:
        ...
        ALLOWED_CITIES = list_pool_cities()
    except Exception:
