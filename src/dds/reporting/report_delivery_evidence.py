...
def _canonical_https_url(value: Any) -> str:
    ...
    netloc = f"{host_for_url}:443" if port == 443 else host_for_url
...
def _web_locator(source: Mapping[str, Any]) -> str:
    for field in (
        "canonical_url",
        "url",
        "canonical_ref",
...
def _source_kind(source: Mapping[str, Any]) -> str:
    return "web" if _web_locator(source) else "local"
...
def _dns_answer_values(value: Any) -> list[str]:
    if isinstance(value, Mapping):
        value = value.get("addresses") or value.get("ips") or []
...
def _verified_public_ips(
    hostname: str,
    resolver: Callable[[str, int], Any] | None,
) -> frozenset[str]:
    try:
        answers = (
            resolver(hostname, 443)
            if resolver is not None
            else socket.getaddrinfo(hostname, 443, type=socket.SOCK_STREAM)
        )
    except Exception as exc:
        raise ReportDeliveryEvidenceError("web_dns_resolution_failed") from exc

    verified: set[str] = set()
    for raw_address in _dns_answer_values(answers):
        try:
            address = ipaddress.ip_address(raw_address)
        except ValueError as exc:
            raise ReportDeliveryEvidenceError("web_dns_invalid_address") from exc
        ...
        if (
            ...
            raise ReportDeliveryEvidenceError("web_dns_address_forbidden")
        verified.add(address.compressed)
    if not verified:
        raise ReportDeliveryEvidenceError("web_dns_resolution_empty")
    return frozenset(sorted(verified))


def _transport_response(response: Any) -> tuple[int, bytes, str, str]:
    status: Any = 200
        ...
        if isinstance(headers, Mapping):
            for key, value in headers.items():
                if str(key).casefold() == "location":
                    ...
                    break
        location = str(response.get("location") or location).strip()
        reported_url = str(
            response.get("final_url") or response.get("url") or ""
        ).strip()
    elif isinstance(response, tuple) and len(response) == 2:
        payload, reported_url = response
    try:
        status_code = int(status)
    except (TypeError, ValueError) as exc:
        raise ReportDeliveryEvidenceError("web_transport_invalid_status") from exc
    if isinstance(payload, str):
        payload = payload.encode("utf-8")
    if not isinstance(payload, (bytes, bytearray)):
        raise ReportDeliveryEvidenceError("web_fetch_invalid_response")
    body = bytes(payload)
    if len(body) > _MAX_WEB_BYTES:
        raise ReportDeliveryEvidenceError("web_response_too_large")
    return status_code, body, location, reported_url


def _fetch_web_bytes(
    url: str,
    transport: Callable[[str, frozenset[str]], Any],
    *,
    resolver: Callable[[str, int], Any] | None,
    max_redirects: int,
) -> tuple[bytes, str, tuple[str, ...]]:
    current = _canonical_https_url(url)
    redirect_limit = min(10, max(0, int(max_redirects)))
    chain = [current]
    redirect_count = 0
    ...
    while True:
        ...
        if not hostname:
            raise ReportDeliveryEvidenceError("web_url_not_https")
        verified_ips = _verified_public_ips(hostname, resolver)
        try:
            response = transport(current, verified_ips)
        except TypeError as exc:
            raise ReportDeliveryEvidenceError(
                "web_transport_must_accept_verified_ips"
            ) from exc
        except Exception as exc:
            raise ReportDeliveryEvidenceError("web_fetch_failed") from exc

        status, body, location, reported_url = _transport_response(response)
        if reported_url:
            try:
                reported_canonical = _canonical_https_url(reported_url)
            except ValueError as exc:
                raise ReportDeliveryEvidenceError(str(exc)) from exc
            if reported_canonical != current:
                raise ReportDeliveryEvidenceError(
                    "web_transport_followed_unvalidated_redirect"
                )

        if 300 <= status < 400:
            if not location:
                raise ReportDeliveryEvidenceError("web_redirect_location_missing")
            if redirect_count >= redirect_limit:
                raise ReportDeliveryEvidenceError("web_redirect_limit_exceeded")
            redirect_count += 1
            try:
        ...
        if not 200 <= status < 300:
            raise ReportDeliveryEvidenceError("web_fetch_http_status")
        return body, current, tuple(chain)
...
def _validate_source(
    ...
    *,
    ...
    project_id: str,
    phase: str,
    transport: Callable[[str, frozenset[str]], Any] | None,
    dns_resolver: Callable[[str, int], Any] | None,
    max_redirects: int,
    ...
    if kind == "web":
        candidate_url = _web_locator(source)
        try:
        ...
        fetch_details: dict[str, Any] = {}
        if phase == "freeze" and canonical_ref:
            if transport is None:
                errors.append("web_transport_required")
            else:
                try:
                    fetched, final_url, redirect_chain = _fetch_web_bytes(
                        canonical_ref,
                        transport,
                    ...
                    fetch_details = {
                        ...
                        "redirect_chain_hash": _hash(list(redirect_chain)),
                    }
    else:
        try:
    ...
    return (
        {
            ...
            **(fetch_details if kind == "web" else {}),
        },
        [],
    )
...
def validate_report_delivery_evidence(
    ...
    *,
    ...
    phase: str = "freeze",
    fetcher: Callable[[str, frozenset[str]], Any] | None = None,
    prior_validation: Mapping[str, Any] | None = None,
    transport: Callable[[str, frozenset[str]], Any] | None = None,
    dns_resolver: Callable[[str, int], Any] | None = None,
    max_redirects: int = _DEFAULT_MAX_REDIRECTS,
) -> dict[str, Any]:
    """Validate candidates/package and return a deterministic DDS attestation.

    Web locators are fail-closed unless an explicit two-argument transport is
    supplied. Every request and redirect is HTTPS-only, DNS-validated and bound
    to the verified public IP set passed to that transport. ``compile`` remains
    offline and verifies the prior content-addressed DDS attestation.
    """
    if not isinstance(evidence, Mapping):
        raise TypeError("evidence must be a mapping")
    if fetcher is not None and transport is not None and fetcher is not transport:
        raise ValueError("provide only one of fetcher or transport")
    selected_transport = transport or fetcher
    ...
    for source in sorted(sources, key=lambda row: str(row.get("source_id") or "")):
        attestation, row_errors = _validate_source(
            ...
            phase=source_phase,
            transport=selected_transport if phase == "freeze" else None,
        ...
        result.update(
            {
                ...
                "claim_ids": sorted(set(claim_ids)),
                "network_policy": (
                    "https_ip_pinned_refetched_and_frozen"
                    if phase == "freeze"
                    else "offline_attestation_verification"
                ),
            }
        )
    result["validation_hash"] = _hash(result)
