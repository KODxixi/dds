...
class OrchestratorAgent(BaseAgent):
    ...
    async def _generate_report(self, task: AgentTask) -> AgentResult:
        ...
        validation = dict(audit_result.data)
        validation["warnings"] = list(state.warnings)
    ...
    async def _run_subtask(
        ...
        for attempt in range(task.max_retries + 1):
            ...
            if result.success:
                self._merge_warnings(state, result.warnings)
                return result
        ...
        self._merge_warnings(state, final_result.warnings)
        return final_result

    @staticmethod
    def _merge_warnings(state: ReportGenerationState, warnings: list[str]) -> None:
        """Collect unique sub-agent warnings in first-seen order."""
