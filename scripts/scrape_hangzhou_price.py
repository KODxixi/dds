"""
杭州市商品房预售价格备案公示（一房一价）抓取脚本

【URL 探测结果记录】
- https://loushi.hzfc.gov.cn/ -> ReadTimeout
- https://loushi.hzfc.gov.cn/yclb/ -> ReadTimeout
- https://zjj.hangzhou.gov.cn/ -> ReadTimeout
- https://zfcxjst.zj.gov.cn/ -> ReadTimeout

由于官方 URL 当前无法直接通过脚本访问（可能存在 WAF/Geoblock 或网络限制），
此脚本处于"结构完整但待填入真实 URL"的状态。

【下一步操作指南】
1. 使用本地浏览器手动打开杭州市住房保障和房产管理局等官方网站。
2. 找到"一房一价"或"预售价格备案"、"商品房信息公示"栏目。
3. 打开浏览器开发者工具 (F12) -> Network 面板，点击查询或翻页。
4. 找到返回楼盘列表及具体房源价格的 XHR/Fetch JSON 请求（通常带有 page/size 等参数）。
...
SOURCE = "hangzhou_price"
CATEGORY = "price_registration"
TZ = timezone(timedelta(hours=8))
UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36"

# T7: 从环境变量读 URL，未配置时优雅降级
BASE_URL = os.getenv("HZ_PRICE_URL", "").rstrip("/") if os.getenv("HZ_PRICE_URL") else None
if not BASE_URL:
    print("[WARN] HZ_PRICE_URL not configured in environment")
    print("[WARN] Set HZ_PRICE_URL in .env and retry, or see README for manual setup")
    sys.exit(0)  # 优雅退出

LIST_API_PATH = "/api/v1/projects"    # 替换为真实楼盘列表接口
DETAIL_API_PATH = "/api/v1/rooms"     # 替换为真实房源明细接口
...
def main() -> int:
    parser = argparse.ArgumentParser(description="杭州商品房预售价格备案抓取")
    ...
    parser.add_argument("--out",       default=r"C:\Users\shiguanyu\DDS\data_out\hangzhou_price")
    ...
    for page in range(start_page, total_pages + 1):
        try:
            if "TODO" not in BASE_URL:
                ...
                # Demo 数据，用于验证架构和文件落地
                raw_items = [{
                    "district": "西湖区", "projectName": "测试楼盘A", "developer": "测试房产",
                    "address": "文一西路", "permitNo": "杭售许字(2023)第00001号",
                    "buildingNo": "1幢", "unitNo": f"{i}01室", "floor": str(i),
                    "roomType": "四室两厅", "area": 120.5, "totalPrice": 4820000,
                    "unitPrice": 40000, "regDate": "2023-10-01", "status": "在售"
                } for i in range(1, 11)]
        ...
        if not args.no_upload:
            gcs_base = f"{args.bucket}/real_estate_data/provincial/hangzhou/{CATEGORY}"
            try:
