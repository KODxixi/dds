...
def main():
    ...
    for i, suite in enumerate(suites, 1):
        ...
        try:
            ...
            # 物理拆弹：将多线程计算库全部强制单线程化，彻底消除 Windows 平台下 pyarrow 与 torch OpenMP 的 DLL 符号竞争与 access violation 崩溃！
            my_env["OMP_NUM_THREADS"] = "1"
            my_env["MKL_NUM_THREADS"] = "1"
            my_env["OPENBLAS_NUM_THREADS"] = "1"
            my_env["VECLIB_MAXIMUM_THREADS"] = "1"
            my_env["NUMEXPR_NUM_THREADS"] = "1"
            res = subprocess.run(suite['command'], check=False, env=my_env)
